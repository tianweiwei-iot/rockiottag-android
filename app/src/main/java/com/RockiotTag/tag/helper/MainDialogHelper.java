...
import com.RockiotTag.tag.AvatarAdapter;
import com.RockiotTag.tag.EditProfileActivity;
...
public class MainDialogHelper {
    ...
    private MaterialAlertDialogBuilder createAlertDialogBuilder() {
        ...
        ContextThemeWrapper themedContext = new ContextThemeWrapper(activity, R.style.AlertDialogTheme);
        ...
        AlertDialog mapDialog = createAlertDialogBuilder()
                ...
                .setSingleChoiceItems(createSingleChoiceAdapter(mapOptions), checkedItem, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        android.content.SharedPreferences prefs = activity.getSharedPreferences("app_settings", android.content.Context.MODE_PRIVATE);
                        String currentMap = prefs.getString("map_provider", "amap");
                        if (newMapProvider.equals(currentMap)) {
                            dialog.dismiss();
                            return;
                        }
                        ...
                        dialog.dismiss();
                        confirmMapProviderSwitch(newMapProvider, toastMessage);
                    }
                })
                .setNegativeButton(R.string.cancel, null)
                .create();
    ...
    private void confirmMapProviderSwitch(String newMapProvider, int toastMessage) {
        boolean showWarning = com.RockiotTag.tag.map.MapAdapterFactory.PROVIDER_GOOGLE.equals(newMapProvider)
                && com.RockiotTag.tag.map.MapAdapterFactory.isAmapRecommended();
        int messageRes = R.string.map_switch_warn_google_in_china;
        if (com.RockiotTag.tag.map.MapAdapterFactory.PROVIDER_AMAP.equals(newMapProvider)
                && com.RockiotTag.tag.map.MapAdapterFactory.isGoogleRecommended()) {
            showWarning = true;
            messageRes = R.string.map_switch_warn_amap_abroad;
        }
        if (showWarning) {
            createAlertDialogBuilder()
                    .setMessage(messageRes)
                    .setPositiveButton(R.string.confirm, (d, w) ->
                            callbacks.onMapProviderChanged(newMapProvider, toastMessage))
                    .setNegativeButton(R.string.cancel, null)
                    .show();
        } else {
            callbacks.onMapProviderChanged(newMapProvider, toastMessage);
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                if (item.code.equals(currentLanguageCode)) {
                    if (darkMode) {
                        ...
                        view.setBackgroundColor(activity.getResources().getColor(R.color.brand_primary, null));
                    }
    ...
    public void showVersionInfo() {
        try {
            ...
            ToastHelper.show(activity, "Error: " + e.getMessage());
        }
    }
    ...
    public void showPermissionRationale(final String[] permissions, final int requestCode) {
        createAlertDialogBuilder()
                ...
                .setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        ToastHelper.show(activity, R.string.permission_denied);
                    }
                })
                .show();
    }
    ...
    public void showLoginDialog() {
        Context dialogContext = isDarkModeEnabled()
                ? new ContextThemeWrapper(activity, R.style.AlertDialogTheme_Dark)
                : new ContextThemeWrapper(activity, R.style.AlertDialogTheme);
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(dialogContext);
        ...
        dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (!UserApiService.isValidUsername(username)) {
                ToastHelper.show(activity, R.string.username_invalid);
                return;
            }
            if (!UserApiService.isValidPassword(password)) {
                ToastHelper.show(activity, R.string.password_invalid);
                return;
            }
                ...
                activity.runOnUiThread(() -> {
                    if (response.isSuccess() && response.getToken() != null) {
                        ...
                        activity.getSharedPreferences("app_settings", android.content.Context.MODE_PRIVATE).edit()
                            ...
                            .apply();
                        ToastHelper.show(activity, R.string.login_success);
                        dialog.dismiss();
                        // 更新ProfileFragment
                        ...
                        if (msg != null && msg.contains("not found")) {
                            ToastHelper.show(activity, R.string.user_not_found);
                        } else if (msg != null && msg.contains("password")) {
                            ToastHelper.show(activity, R.string.wrong_password);
                        } else {
                            ToastHelper.show(activity, R.string.login_failed);
                        }
                    }
                });
            }).start();
        ...
        dialog.getButton(android.app.AlertDialog.BUTTON_NEUTRAL).setOnClickListener(v -> {
            ToastHelper.showLong(activity, R.string.reset_password_contact_admin);
        });
    }
    ...
    public void showRegisterDialog() {
        ...
        // 头像选择
        TextView avatarLabel = new TextView(activity);
        avatarLabel.setText(activity.getString(R.string.select_avatar_register));
        avatarLabel.setTextSize(14);
        avatarLabel.setTextColor(activity.getResources().getColor(R.color.text_secondary, null));
        layout.addView(avatarLabel);

        final int[] selectedAvatarIndex = {-1};
        RecyclerView avatarGrid = new RecyclerView(activity);
        GridLayoutManager avatarLayoutManager = new GridLayoutManager(activity, 4) {
            @Override
            public boolean canScrollVertically() {
                return false;
            }
        };
        avatarGrid.setLayoutManager(avatarLayoutManager);
        avatarGrid.setNestedScrollingEnabled(false);
        avatarGrid.setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));
        AvatarAdapter avatarAdapter = new AvatarAdapter(EditProfileActivity.AVATAR_RESOURCES, -1);
        avatarAdapter.setOnAvatarClickListener(position -> {
            selectedAvatarIndex[0] = position;
            avatarAdapter.setSelectedIndex(position);
        });
        avatarGrid.setAdapter(avatarAdapter);
        layout.addView(avatarGrid);
        ...
        dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (!password.equals(confirmPassword)) {
                ToastHelper.show(activity, R.string.password_not_match);
                return;
            }

            final int avatarIdx = selectedAvatarIndex[0];
            new Thread(() -> {
                NewApiService.ApiResponse response = UserApiService.getInstance().register(username, password);
                activity.runOnUiThread(() -> {
                    if (response.isSuccess()) {
                        ToastHelper.show(activity, R.string.register_success);
                        // 注册成功后自动登录
                        new Thread(() -> {
                            ...
                            activity.runOnUiThread(() -> {
                                if (loginResp.isSuccess() && loginResp.getToken() != null) {
                                    ...
                                    BoundDevicesHelper.clearBoundDevicesCache(activity);
                                    android.content.SharedPreferences.Editor editor = activity.getSharedPreferences("app_settings", android.content.Context.MODE_PRIVATE).edit();
                                    editor.putString("auth_token", token)
                                        .putString("user_username", username)
                                        .remove("selected_device_id");
                                    if (avatarIdx >= 0) {
                                        editor.putInt("user_avatar_index", avatarIdx);
                                    }
                                    editor.apply();
                                    // 同步头像到服务器
                                    if (avatarIdx >= 0) {
                                        new Thread(() -> {
                                            try {
                                                UserApiService.getInstance().updateAvatar(token, String.valueOf(avatarIdx));
                        ...
                        if (msg != null && (msg.contains("exist") || msg.contains("already"))) {
                            ToastHelper.show(activity, R.string.username_exists);
                        } else {
                            ToastHelper.show(activity, R.string.register_failed);
                        }
                    }
                });
            }).start();
